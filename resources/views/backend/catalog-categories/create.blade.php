...
@section('title', __('labels.backend.access.catalog-categories.management') . ' | ' . __('labels.backend.access.catalog-categories.create'))
...
@section('content')
{{ Form::open(['route' => 'admin.catalog-categories.store', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post', 'id' => 'create-permission', 'files' => true]) }}
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-sm-5">
                                ...
                                @foreach(cache()->get('languages') as $language)
                                    <div class="tab-pane fade @if($loop->first) show active @endif" id="nav-name-{{$language->locale}}" role="tabpanel" aria-labelledby="nav-name-{{$language->locale}}-tab">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-10">
                                                <input name="translations[{{$language->locale}}][name]" id="name_{{$language->locale}}" type="text" class="form-control" placeholder="@lang('validation.attributes.backend.access.catalog-categories.name')">
                                            </div><!--col-->
                                        </div><!--form-group-->
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-10">
                                                <textarea name="translations[{{$language->locale}}][desc]" id="desc_{{$language->locale}}" rows="6" class="form-control" placeholder="@lang('validation.attributes.backend.access.catalog-categories.desc')"></textarea>
                                            </div><!--col-->
                                        </div><!--form-group-->
                                    </div>
                                @endforeach
        ...
        @include('backend.components.footer-buttons', [ 'cancelRoute' => 'admin.catalog-categories.index' ])
    </div><!--card-->
    {{ Form::close() }}
@endsection
