...
@section('title', __('labels.backend.access.partners.management') . ' | ' . __('labels.backend.access.partners.create'))
...
@section('content')
{{ Form::open(['route' => 'admin.partners.store', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post', 'id' => 'create-permission', 'files' => true]) }}
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-sm-5">
                                ...
                                @foreach(cache()->get('languages') as $language)
                                    <div class="tab-pane fade @if($loop->first) show active @endif" id="nav-name-{{$language->locale}}" role="tabpanel" aria-labelledby="nav-name-{{$language->locale}}-tab">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-10">
                                                <input name="translations[{{$language->locale}}][name]" id="name_{{$language->locale}}" type="text" class="form-control" placeholder="@lang('validation.attributes.backend.access.partners.name')">
                                            </div><!--col-->
                                        </div><!--form-group-->
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-10">
                                                <textarea name="translations[{{$language->locale}}][desc]" id="desc_{{$language->locale}}" rows="6" class="form-control tinyText" placeholder="@lang('validation.attributes.backend.access.partners.desc')"></textarea>
                                            </div><!--col-->
                                        </div><!--form-group-->
                                    </div>
                                @endforeach
        ...
        @include('backend.components.footer-buttons', [ 'cancelRoute' => 'admin.partners.index' ])
    </div><!--card-->
    {{ Form::close() }}
@endsection
