...
@section('title', __('labels.backend.access.partners.management') . ' | ' . __('labels.backend.access.partners.edit'))
...
@section('content')
    <form action="{{ route('admin.partners.update',$partner) }}" enctype="multipart/form-data" method="POST" class="form-horizontal">
        @csrf
        @method("PATCH")
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-5">
                        <h4 class="card-title mb-0">
                            {{ __('labels.backend.access.partners.management') }}
                            <small class="text-muted">{{ __('labels.backend.access.partners.create') }}</small>
                        </h4>
                    </div><!--col-->
                </div><!--row-->
                <hr>
                    ...
                    <div class="col">
                        ...
                        <div class="tab-content border-left-0 border-bottom-0 border-right-0" id="nav-tabContent-1">
                                    ...
                                    @foreach(cache()->get('languages') as $language)
                                        <div class="tab-pane fade @if($loop->first) show active @endif" id="nav-name-{{$language->locale}}" role="tabpanel" aria-labelledby="nav-name-{{$language->locale}}-tab">
                                            <div class="form-group row">
                                                <label for="name_{{$language->locale}}" class="col-sm-2 col-form-label">@lang('validation.attributes.backend.access.partners.name')</label>
                                                <div class="col-md-10">
                                                    <input name="translations[{{$language->locale}}][name]" id="name_{{$language->locale}}" type="text" class="form-control" value="{{ $partner->translate($language->locale)->name ?? '' }}">
                                                </div><!--col-->
                                            </div><!--form-group-->
                                            <div class="form-group row">
                                                <label for="desc_{{$language->locale}}" class="col-sm-2 col-form-label">@lang('validation.attributes.backend.access.partners.desc')</label>
                                                <div class="col-md-10">
                                                    <textarea name="translations[{{$language->locale}}][desc]" id="desc_{{$language->locale}}" rows="6" class="form-control tinyText">{{ $partner->translate($language->locale)->desc ?? '' }}</textarea>
                                                </div><!--col-->
                                            </div><!--form-group-->
                                        </div>
                                    @endforeach
                            ...
                            <div class="tab-pane fade" id="nav-data" role="tabpanel" aria-labelledby="nav-data-tab">
                                <div class="form-group row">
                                    <label for="rank" class="col-sm-2 col-form-label">@lang('validation.attributes.backend.access.partners.rank')</label>
                                    <div class="col-md-10">
                                        <input type="number"  name="rank" class="form-control" id="rank"  value="{{ $partner->rank }}">
                                    </div>
                                    <!--col-->
                                </div>
                                <!--form-group-->
                                ...
                                <div class="form-group row">
                                    <label for="active" class="col-sm-2 col-form-label">@lang('validation.attributes.backend.access.partners.status')</label>
                                    <div class="col-md-10">
                                        <div class="checkbox d-flex align-items-center">
                                            <label class="switch switch-label switch-pill switch-primary mr-2" for="role-1">
                                                <input class="switch-input" type="checkbox" name="active" id="role-1" value="1" @if($partner->active) checked @endif>
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <table>
                                            ...
                                            <tr>
                                                <td>
                                                    <img src="/{{ $partner->album->image->path ?? '' }}" width="200" class="img-thumbnail">
                                                </td>
                                                <td class="ml-2">
